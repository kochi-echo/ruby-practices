...
class TestFrameMethod < Minitest::Test
  ...
  def test_throw_max_each_frame
    frame = Frame.new
    frame.points = [3]
    refute frame.throw_max_each_frame?(1, 20)
    frame = Frame.new
    frame.points = [2, 8]
    assert frame.throw_max_each_frame?(2, 20)
    frame = Frame.new
    frame.points = [10]
    assert frame.throw_max_each_frame?(1, 20)
    ...
    frame.points = [2, 8]
    frame.frame_num = 10
    refute frame.throw_max_each_frame?(2, 20)
    frame = Frame.new
    frame.points = [10]
    frame.frame_num = 10
    refute frame.throw_max_each_frame?(1, 20)
    ...
    frame.points = [2, 8, 3]
    assert frame.throw_max_each_frame?(19, 20)
  end
end
